...
export default function TabRealisasiBiaya() {
  ...
  const [sunting, setSunting] = useState<RealisasiEntry | null>(null)
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      // Perubahan dari AI DIPERIKSA dulu, dan yang dihitung hanya yang
      // sungguh terjadi.
      //
      // Dulu baris ini menerapkan tambalan mentah-mentah lalu menambahkan
      // `updated.length` ke penghitung. Dua akibatnya sama-sama diam: id yang
      // tidak ada membuat `map` tidak menemukan apa pun tanpa melempar galat,
      // dan nominal berupa teks "135.000" tersimpan sebagai 135 karena
      // JavaScript membaca titiknya sebagai desimal. Notifikasinya tetap
      // berbunyi "✅ perubahan dicatat" pada kedua keadaan itu.
      let pesanSunting = ''
      let countRevisi = 0
      if (parsedResult.updated.length > 0) {
        const h = terapkanPerubahan(realisasiEntries, parsedResult.updated, MEDAN_BIAYA)
        for (const id of h.berubah) {
          const baris = h.hasil.find(e => e.id === id)
          if (baris) updateRealisasiEntry(id, baris)
        }
        changeCount += h.berubah.length
        countRevisi = h.berubah.length
        if (h.hilang.length || h.kosong.length) pesanSunting = kalimatSunting(h)
      }
      ...
      // Kegagalan revisi diberitahukan, meski ada perubahan lain yang berhasil.
      // Yang paling merugikan bukan gagalnya, melainkan gagal tanpa dikatakan.
      if (pesanSunting) {
        toast({ title: 'Sebagian revisi tidak masuk', description: pesanSunting, variant: 'destructive' })
      }
      if (changeCount > 0) {
        toast({
          title: `✅ ${changeCount} perubahan dicatat!`,
          description: `${parsedResult.added.length} baru, ${countRevisi} direvisi, ${parsedResult.deleted.length} dihapus`
        })
      }
    } catch (err: any) {
  ...
  return (
    <div className="flex flex-col lg:flex-row gap-4 lg:gap-5 lg:min-h-[600px] lg:max-h-[90vh]">
              ...
              [...filteredEntries].reverse().map(e => {
                ...
                return (
                  <div key={e.id} className="border border-border rounded-xl p-3 bg-white hover:bg-muted/20 transition-colors">
                    {/* Seluruh kartunya bisa diketuk untuk diperbaiki.
                        Sampai sekarang satu-satunya cara mengubah sebuah baris
                        adalah menyuruh AI — jadi ketika lima baris tercatat
                        Rp 135 padahal maksudnya Rp 135.000, yang bisa dilakukan
                        hanyalah mengetik ulang permintaannya dan berharap kali
                        ini dimengerti. Salah ketik satu angka tidak seharusnya
      ...
      {sunting && (
        <SuntingBiaya entri={sunting}
          onSimpan={patch => updateRealisasiEntry(sunting.id, patch)}
          onHapus={() => deleteRealisasiEntry(sunting.id)}
